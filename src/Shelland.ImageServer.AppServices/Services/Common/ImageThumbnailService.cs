...
public class ImageThumbnailService : IImageThumbnailService
{
    ...
    public async Task<ImageUploadResultModel> ProcessThumbnails(ImageUploadJob uploadJob, CancellationToken cancellationToken)
    {
        ...
        if (this.appSettings.Value.Common.SaveOriginalFile)
        {
            ...
            originalFileUrl = this.linkService.PrepareWebPath(storagePath.UrlPath);
        }
        ...
        var dbModel = await this.imageUploadDataService.Create(storagePath, uploadJob, thumbnails);
        ...
        await this.mediator.Send(new ImageProcessingFinishedPayload(result), cancellationToken);
    ...
    /// <returns></returns>
    private async Task<IReadOnlyCollection<ImageThumbnailParamsModel>> GetThumbnailParams(ImageUploadJob uploadJob)
    {
